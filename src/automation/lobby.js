    ...
    window.adTourney.startMatchDirectly = async function(p1Name, p2Name, matchObj) {
        ...
        if (!token) {
            window.adModals.show({
                title: 'TOKEN FEHLT',
                text: 'Bitte öffne kurz eine beliebige andere Seite auf autodarts.io / autodarts.com und komm zurück, dann sollte es funktionieren.',
                confirmText: 'OK'
            });
            return;
        }
        ...
        try {
            ...
            const boardRes = await fetch(window._AD_API+'/bs/v0/boards', { headers });
        ...
        async function performLobbyCreation(boardId) {
            try {
                ...
                // 1. LOBBY ERSTELLEN
                const createRes = await fetch(window._AD_API+'/gs/v0/lobbies', {
                    method: 'POST',
                    headers,
                    body: JSON.stringify({
                ...
                const addPlayer = (name, bId) => {
                    ...
                    return fetch(window._AD_API+'/gs/v0/lobbies/'+lobbyId+'/players', {
                        method: 'POST',
                        headers,
                ...
                // 3. START
                const startRes = await fetch(window._AD_API+'/gs/v0/lobbies/'+lobbyId+'/start', {
                    method: 'POST',
                    headers
                });
                ...
                console.log('[AD-Lobby] ✅ Weiterleitung zu:', window._AD_PLAY+'/matches/'+lobbyId);
                window.location.href = window._AD_PLAY+'/matches/'+lobbyId;
